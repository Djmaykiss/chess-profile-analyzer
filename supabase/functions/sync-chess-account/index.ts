...
Deno.serve(async request => {
  ...
  let admin: any; let runId: string | undefined
  try {
    ...
    const { data: account } = await admin.from('chess_accounts').select('*').eq('id', accountId).eq('profile_id', profileId).eq('verification_status', 'verified').eq('is_active', true).single()
    if (!account) return Response.json({ error: 'Cuenta no sincronizable' }, { status: 404, headers: cors })
    const { data: run, error: runError } = await admin.from('sync_runs').insert({ profile_id: profileId, account_id: accountId, status: 'running' }).select().single()
    ...
    if (account.platform === 'chesscom') {
      ...
      const response = await fetchWithRetry(`https://lichess.org/api/games/user/${encodeURIComponent(account.username)}?pgnInJson=true&clocks=false${since ? `&since=${since}` : ''}`, { headers: { Accept: 'application/x-ndjson' } })
  ...
  } catch (_error) { if (admin && runId) await admin.from('sync_runs').update({ status: 'failed', finished_at: new Date().toISOString(), error_message: safeError }).eq('id', runId); return Response.json({ error: safeError }, { status: 500, headers: cors }) }
})
