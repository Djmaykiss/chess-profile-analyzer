...
import { playedAt, splitNdjson } from './normalizers.ts'
...
async function importLichessStream(response: Response, account: any, profileId: string, admin: any, runId: string) {
  ...
  const reader = response.body.getReader(); const decoder = new TextDecoder(); let remainder = ''; let found = 0; let batch: any[] = []
  ...
  while (true) {
    ...
    for (const line of split.complete) { const game = JSON.parse(line); if (!game.pgn || (game.variant && game.variant !== 'standard')) continue; batch.push(toGame(account, profileId, game.id, game.pgn, game)); found += 1; if (batch.length >= 200) await flush() }
    if (done) break
  }
  if (remainder.trim()) { const game = JSON.parse(remainder); if (game.pgn && (!game.variant || game.variant === 'standard')) { batch.push(toGame(account, profileId, game.id, game.pgn, game)); found += 1 } }
  await flush(); return found
}
...
Deno.serve(async request => {
  ...
  try {
    ...
    if (account.platform === 'chesscom') {
      ...
      found = await importLichessStream(response, account, profileId, admin, runId)
    }
    ...
    await admin.from('sync_runs').update({ status: 'completed', finished_at: completedAt, games_found: found, games_imported: imported, games_skipped: found - imported }).eq('id', runId)
    return Response.json({ runId, found, imported, skipped: found - imported }, { headers: cors })
