...
function OpeningRows({ rows }: { rows: OpeningStat[] }) { return rows.length ? <div className="opening-table"><div className="opening-head"><span>Apertura</span><span>Partidas</span><span>V/T/D</span><span>Win rate</span></div>{rows.map(row => { const labels = openingLabels(row.eco, row.opening); return <div className="opening-row" key={`${row.eco}-${row.opening}`}><span><b>{labels.eco}</b><small>{labels.opening}</small></span><span>{row.games}</span><span>{resultLabel(row)}</span><span>{percent(row)}%</span></div> })}</div> : <p className="subtle-copy">No hay aperturas con datos reales en este rango.</p> }
