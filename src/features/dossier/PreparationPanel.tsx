...
const Sentence = ({ row, kind }: { row: ScoutingEvidence; kind: string }) => { const labels = openingLabels(row.eco, row.opening); return <li key={`${kind}-${row.move_sequence ?? row.opening}`}><strong>{kind}:</strong> {row.move_sequence ?? `${labels.eco} · ${labels.opening}`} aparece en {row.games} partidas ({row.win_rate}% victorias; {row.confidence}).</li> }
