...
import { DossierRange, RepertoireNode } from './dossier.types'
import { percent, resultLabel } from './dossier-formatters'
import { useBuildProfileRepertoireIndex, useProfileRepertoireTree } from './dossier.hooks'
...
export function RepertoireTree({ profileId, range }: { profileId: string; range: DossierRange }) {
  const [color, setColor] = useState<'white' | 'black'>('white'); const [maxPly, setMaxPly] = useState(8); const [minGames, setMinGames] = useState(10); const [expanded, setExpanded] = useState<Set<string>>(new Set(['']))
  ...
  const continueLabel = build.isPending ? 'Preparando lote…' : build.data && !build.data.completed ? 'Continuar indexación' : 'Preparar repertorio'
  const buildButton = <button className="secondary compact" disabled={build.isPending || build.data?.completed} onClick={() => build.mutate()}>{continueLabel}</button>
  return <section className="card repertoire-section"><div className="card-title"><div><p className="eyebrow">PGN REALES · LÍNEA PRINCIPAL</p><h2>Repertorio real</h2></div><div className="repertoire-controls"><select value={color} onChange={event => setColor(event.target.value as 'white' | 'black')}><option value="white">Blancas</option><option value="black">Negras</option></select><select value={maxPly} onChange={event => setMaxPly(Number(event.target.value))}>{[4,6,8,10,12].map(value => <option key={value} value={value}>Hasta {value} ply</option>)}</select><select value={minGames} onChange={event => setMinGames(Number(event.target.value))}>{[3,5,10,20].map(value => <option key={value} value={value}>Mín. {value} partidas</option>)}</select></div></div>{build.data && <div className="repertoire-progress"><span>{build.data.completed ? `Índice completo: ${build.data.processedGames} partidas.` : `Lote listo: ${build.data.processedGames} partidas indexadas.`}</span>{buildButton}</div>}{isLoading ? <p className="subtle-copy">Cargando ramas agregadas…</p> : error ? <div className="repertoire-empty"><p>El índice todavía no está disponible para este perfil.</p>{buildButton}{build.isError && <p className="form-error">No se pudo preparar el repertorio.</p>}</div> : nodes.length ? <><div className="repertoire-tree"><TreeBranch parent="" depth={0} tree={tree} expanded={expanded} toggle={toggle}/></div>{!build.data && <div className="repertoire-empty"><p>El índice contiene datos parciales o anteriores.</p>{buildButton}</div>}</> : <div className="repertoire-empty"><p>Aún no hay movimientos indexados para este rango.</p>{buildButton}</div>}</section>
}
