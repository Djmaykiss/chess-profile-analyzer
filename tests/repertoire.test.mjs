...
const aliasFix = readFileSync(new URL('../supabase/migrations/202608120015_fix_repertoire_tree_output_aliases.sql', import.meta.url), 'utf8')
...
for (const required of ['node_sequence', 'node_san', 'node_ply', 'from filtered f', 'f.grouped_games']) assert.ok(aliasFix.includes(required), `Missing repertoire alias repair: ${required}`)
